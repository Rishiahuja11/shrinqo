...
public class BotManager {
    ...
    /** Names of all fake players across instances, so bots ignore each other. */
    static final java.util.Set<String> FAKE_NAMES = ConcurrentHashMap.newKeySet();
    ...
    private void startTasks() {
        ...
        }.runTaskTimer(plugin, 40L, 20L + rng.nextInt(15));
        ...
        lookTask = new BukkitRunnable() {
            @Override public void run() {
                if (!active) { cancel(); return; }
                try { lookAtNearestPlayer(); } catch (Exception ignored) {}
            }
        }.runTaskTimer(plugin, 60L, 40L + rng.nextInt(40));
    }
    ...
    private void moveRandom() {
        ...
        // Occasionally idle like a real player
        if (rng.nextInt(4) == 0) return;

        double dx, dz;
        Player target = (rng.nextInt(3) == 0) ? nearestRealPlayer(20) : null;
        if (target != null) {
            // drift toward the real player
            Location tl = target.getLocation();
            dx = tl.getX() - botLoc.getX();
            dz = tl.getZ() - botLoc.getZ();
            double len = Math.sqrt(dx * dx + dz * dz);
            if (len < 2.5 || len == 0) { lookAround(); return; }
            double step = 1.2 + rng.nextDouble() * 1.0;
            dx = (dx / len) * step;
            dz = (dz / len) * step;
        } else {
            dx = (rng.nextDouble() - 0.5) * 3;
            dz = (rng.nextDouble() - 0.5) * 3;
        }
        ...
        if (nmsReady) {
            try {
                ...
                broadcastTeleport();
                broadcastHeadRotation();
                if (rng.nextInt(3) == 0) broadcastAnimation();
                if (ny > botLoc.getY() && rng.nextInt(2) == 0) broadcastMotion(0.42);
            } catch (Exception ignored) {}
        }

        if (rng.nextInt(25) == 0) toggleSneak();
        if (rng.nextInt(35) == 0) toggleSprint();
    }

    private Player nearestRealPlayer(double maxDist) {
        Player best = null;
        double bd = maxDist * maxDist;
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.getName().equals(botName) || FAKE_NAMES.contains(p.getName())) continue;
            Location l = p.getLocation();
            if (world == null || !l.getWorld().equals(world)) continue;
            if (botLoc == null) continue;
            double d = l.distanceSquared(botLoc);
            if (d < bd) { bd = d; best = p; }
        }
        return best;
    }
    ...
    private void lookAtNearestPlayer() {
        if (nmsPlayer == null || !nmsReady || botLoc == null) { lookAround(); return; }
        Player t = nearestRealPlayer(12);
        if (t == null) { lookAround(); return; }
        try {
            Location tl = t.getLocation().add(0, 1.6, 0);
            double dx = tl.getX() - botLoc.getX();
            double dy = tl.getY() - (botLoc.getY() + 1.6);
            double dz = tl.getZ() - botLoc.getZ();
            double horiz = Math.sqrt(dx * dx + dz * dz);
            if (horiz < 0.01) { lookAround(); return; }
            yaw = (float) ((Math.atan2(dz, dx) * 180.0 / Math.PI) + 90.0);
            pitch = (float) (-(Math.atan2(dy, horiz) * 180.0 / Math.PI));
            broadcastHeadRotation();
            if (rng.nextInt(5) == 0) broadcastAnimation();
        } catch (Exception ignored) {}
    }
    ...
    private void broadcastPlayerInfoUpdate(String action, GameProfile profile) {
        try {
            ...
            for (Constructor<?> ctor : pktClass.getDeclaredConstructors()) {
                Class<?>[] pts = ctor.getParameterTypes();
                if (pts.length == 2 && Set.class.isAssignableFrom(pts[0]) && Collection.class.isAssignableFrom(pts[1])) {
                    ctor.setAccessible(true);
                    broadcastPacket(ctor.newInstance(actionSet, Collections.singletonList(nmsPlayer)));
                    return;
                }
            }
        } catch (Exception ignored) {}
    ...
    private void broadcastMotion(double vy) {
        if (nmsPlayer == null || entityId <= 0) return;
        try {
            Class<?> vec3Class = Class.forName("net.minecraft.world.phys.Vec3");
            Object v = vec3Class.getConstructor(double.class, double.class, double.class).newInstance(0, vy, 0);
            for (String cls : new String[]{
                "net.minecraft.network.protocol.game.ClientboundSetEntityMotionPacket",
                "net.minecraft.network.game.ClientboundSetEntityMotionPacket"
            }) {
                try {
                    ...
                    for (Constructor<?> ctor : c.getConstructors()) {
                        ...
                        if (pts.length == 2 && pts[0] == int.class) {
                            broadcastPacket(ctor.newInstance(entityId, v)); return;
                        }
                        if (pts.length == 2 && pts[0].getName().contains("Entity")) {
                            broadcastPacket(ctor.newInstance(nmsPlayer, v)); return;
                        }
                    }
                } catch (ClassNotFoundException ignored) {}
            }
