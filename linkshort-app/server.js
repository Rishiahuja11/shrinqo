...
const DB_PATH = path.join(DB_DIR, 'linkshort.db');
...
if (DatabaseSync) {
  ...
  db.prepare = function(sql) {
    ...
    stmt.run = function() { return stmt.run.apply(stmt, arguments); };
    stmt.get = function() { return stmt.get.apply(stmt, arguments); };
    stmt.all = function() { return stmt.all.apply(stmt, arguments); };
    return stmt;
  };
  const origExec = db.exec.bind(db);
  db.exec = function(sql) {
    if (/^\s*PRAGMA\s+/i.test(sql)) {
      db.pragma(sql.replace(/^\s*PRAGMA\s+/i, '').replace(/;$/,''));
    } else {
      origExec(sql);
    }
  };
} else {
  throw new Error('No SQLite driver available. Install better-sqlite3: npm install better-sqlite3');
}
db.exec('PRAGMA journal_mode = WAL');
db.exec('PRAGMA synchronous = NORMAL');

// Turso write-through: intercept all writes and also send them to Turso via HTTP.
// This ensures data reaches Turso even though better-sqlite3 writes bypass the
// embedded replica's change tracking. Uses raw fetch() — no second @libsql instance.
// NOTE: env vars are read lazily to avoid build-time inlining by esbuild/zisi.
var tursoHttpUrl = '';
var tursoHttpToken = '';

function tursoToArgs(args) {
  var tursoArgs = [];
  if (args && args.length) {
    for (var i = 0; i < args.length; i++) {
      var v = args[i];
      if (v === null || v === undefined) tursoArgs.push({ type: 'null' });
      else if (typeof v === 'number') tursoArgs.push(Number.isInteger(v) ? { type: 'integer', value: String(v) } : { type: 'float', value: String(v) });
      else tursoArgs.push({ type: 'text', value: String(v) });
    }
  }
  return tursoArgs;
}

function tursoSend(sql, args) {
  if (!tursoHttpUrl || !tursoHttpToken) return;
  try {
    var reqs = [{ type: 'execute', stmt: { sql: sql, args: tursoToArgs(args) } }, { type: 'close' }];
    fetch(tursoHttpUrl + '/v2/pipeline', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ' + tursoHttpToken, 'Content-Type': 'application/json' },
      body: JSON.stringify({ requests: reqs })
    }).then(function(r) { return r.json(); }).then(function(j) {
      var err = j.results && j.results[0] && j.results[0].type === 'error';
      if (err) console.error('[turso-write] error:', j.results[0].error && j.results[0].error.message);
    }).catch(function(e) { console.error('[turso-write] failed:', e.message); });
  } catch(e) {}
}

function tursoWrite(sql, args) {
  // Read env vars lazily (first call) to avoid esbuild build-time inlining
  if (!tursoHttpUrl && !tursoHttpToken) {
    var u = process.env.TURSO_DATABASE_URL || '';
    var t = process.env.TURSO_AUTH_TOKEN || '';
    if (u && t) {
      tursoHttpUrl = u.replace(/^libsql:\/\//, 'https://').replace(/^http:\/\//, 'https://');
      tursoHttpToken = t;
      console.log('[turso-write] HTTP write-through enabled to:', tursoHttpUrl);
    }
  }
  if (!tursoHttpUrl || !tursoHttpToken) return;
  // Split multi-statement SQL on semicolons (Turso HTTP API only accepts one statement)
  if (args && args.length) {
    // Parameterized queries are always single statements
    tursoSend(sql, args);
  } else {
    // DDL may contain multiple statements — split on ';'
    var stmts = sql.split(/;\s*(?:\n|$)/).filter(function(s) { return s.trim().length > 0; });
    stmts.forEach(function(s) { tursoSend(s.trim()); });
  }
}

// Wrap db.exec to also send DDL to Turso
var _origDbExec = db.exec.bind(db);
db.exec = function(sql) {
  _origDbExec(sql);
  if (!/^\s*PRAGMA\s+/i.test(sql)) {
    tursoWrite(sql);
...
// Wrap db.prepare to intercept writes via Proxy (node:sqlite Statement may not persist property overrides)
var _origPrepare = db.prepare.bind(db);
db.prepare = function(sql) {
  var stmt = _origPrepare(sql);
  var isWrite = /^\s*(INSERT|UPDATE|DELETE|REPLACE|CREATE|ALTER|DROP)\b/i.test(sql);
  if (isWrite) {
    ...
    return new Proxy(stmt, {
      get: function(target, prop) {
        if (prop === 'run') {
          return function() {
            var result = origRun.apply(null, arguments);
...
// --- v3 migrations: banner for interstitial ---
try { db.exec('ALTER TABLE links ADD COLUMN banner_url TEXT'); } catch(e) {}
try { db.exec('ALTER TABLE links ADD COLUMN banner_fetched_at TEXT'); } catch(e) {}
db.exec(`
...
const S = {
  ...
  // v3: banner
  setBanner: db.prepare('UPDATE links SET banner_url = ?, banner_fetched_at = ? WHERE id = ?'),
  clearOldBanners: db.prepare("UPDATE links SET banner_url = NULL, banner_fetched_at = NULL WHERE banner_url IS NOT NULL AND banner_fetched_at < ?"),
};
...
const CSP = "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://quge5.com https://nap5k.com https://al5sm.com https://n6wxm.com https://5gvci.com https://6opo.com https://auqot.com https://ekhay.com https://b3mny.com https://ay267.com https://fonts.googleapis.com https://app.netlify.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; img-src 'self' data: https:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://quge5.com https://nap5k.com https://al5sm.com https://n6wxm.com https://5gvci.com https://6opo.com https://e2ertt.com https://auqot.com https://ekhay.com https://b3mny.com https://ay267.com https://app.netlify.com; frame-src https://quge5.com https://nap5k.com https://al5sm.com https://n6wxm.com https://5gvci.com https://6opo.com https://auqot.com https://ekhay.com https://b3mny.com https://ay267.com https://app.netlify.com";
...
// ---- Banner fetcher: grab OG image for interstitial (fire-and-forget) ----
async function fetchAndStoreBanner(id, targetUrl) {
  try {
    var meta = await fetchUrlMeta(targetUrl);
    if (meta && meta.image) {
...
function crawlerPage(link, meta) {
  ...
  var img = (link.banner_url) ? link.banner_url : ((meta && meta.image) ? meta.image : (SITE_URL + '/og-image.png'));
...
// prune banners older than 10 minutes (every 2 min)
setInterval(function () {
  try { S.clearOldBanners.run(new Date(Date.now() - 600000).toISOString()); } catch (e) {}
}, 120000).unref();
// self-ping to keep HF Spaces alive (every 10 min)
...
async function handleApiV1(req, res, pathname, method, ip, jsonBody, rawBody) {
  ...
  if (route === '/shorten' && method === 'POST') {
    ...
    // Fetch banner for new URL links (fire-and-forget)
    if (r.created && row && row.kind === 'url' && row.url) fetchAndStoreBanner(r.id, row.url);
  ...
  try {
    ...
    if (pathname.startsWith('/api/')) {
      ...
      if (pathname === '/api/shorten' && method === 'POST') {
        ...
        // Fetch banner for new links (fire-and-forget)
        if (created) fetchAndStoreBanner(id, target);
